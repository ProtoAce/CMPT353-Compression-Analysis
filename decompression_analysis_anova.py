...
def decompression_time_anova(data, storage_type):
    x1 = data.loc[(data['compression_type'] == 'zlib') & (
        data['storage_type'] == storage_type), 'decompression_time']
...
def decompression_time_posthoc(data, storage_type):
    x1 = data.loc[(data['compression_type'] == 'zlib') & (
        data['storage_type'] == storage_type), 'decompression_time'].reset_index(drop=True)
    x2 = data.loc[(data['compression_type'] == 'gzip') & (
        data['storage_type'] == storage_type), 'decompression_time'].reset_index(drop=True)
    x3 = data.loc[(data['compression_type'] == 'bz2') & (
        data['storage_type'] == storage_type), 'decompression_time'].reset_index(drop=True)
...
def anova_analyis():
    ...
    for storage_type in storage_types:
        ...
        if decompression_time_pvalue < 0.05:
            ...
            decompression_time_posthoc(data, storage_type)
